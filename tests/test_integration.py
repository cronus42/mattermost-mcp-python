...
from mcp_mattermost.api.exceptions import HTTPError, AuthenticationError, NotFoundError


# Check if integration tests are enabled
INTEGRATION_TESTS_ENABLED = os.getenv("MATTERMOST_INTEGRATION_TESTS", "false").lower() == "true"
...
pytestmark = pytest.mark.skipif(
    not INTEGRATION_TESTS_ENABLED or not all([MATTERMOST_URL, MATTERMOST_TOKEN, MATTERMOST_TEAM_ID]),
    ...
    async def test_channel_and_member_management(self):
        ...
        with HTTPXMock() as httpx_mock:
            ...
            async with AsyncHTTPClient(self.base_url, self.token) as client:
                ...
                # Add member
                member = await channels_service.add_channel_member("channel123", "user456")
                assert member.channel_id == "channel123"
                assert member.user_id == "user456"
                
                # Get member
                retrieved_member = await channels_service.get_channel_member("channel123", "user456")
...
class TestLiveIntegration:
    ...
    async def test_authentication_and_user_info(self):
        ...
        async with AsyncHTTPClient(self.base_url, self.token) as client:
            ...
            print(f"Authenticated as user: {current_user.username} (ID: {current_user.id})")
    ...
    async def test_channel_operations(self):
        ...
        async with AsyncHTTPClient(self.base_url, self.token) as client:
            ...
            channels = await channels_service.get_channels_for_team(self.team_id, per_page=10)
    ...
    async def test_post_operations_if_channel_available(self):
        ...
        async with AsyncHTTPClient(self.base_url, self.token) as client:
            ...
            try:
                ...
                updated_post = await posts_service.update_post(created_post.id, update_data)
