...
from mcp_mattermost.models.posts import Post, PostCreate, PostPatch, PostList, Reaction
from mcp_mattermost.models.channels import Channel, ChannelCreate, ChannelPatch, ChannelMember
...
class TestBaseService:
    ...
    async def test_make_list_request_invalid_response(self):
        ...
        with pytest.raises(HTTPError) as exc_info:
            await self.service._make_list_request(
                "GET", "/items", MattermostResponse
            )
    ...
    async def test_convenience_methods(self):
        ...
        self.mock_client.request.assert_called_with(
            method="POST", endpoint="/test", data={"test": "data"}, params=None, headers=None
        )
        
        # Test PUT
        await self.service._put("/test", MattermostResponse, data={"test": "data"})
        self.mock_client.request.assert_called_with(
            method="PUT", endpoint="/test", data={"test": "data"}, params=None, headers=None
        )
        
        # Test PATCH
        await self.service._patch("/test", MattermostResponse, data={"test": "data"})
        self.mock_client.request.assert_called_with(
            method="PATCH", endpoint="/test", data={"test": "data"}, params=None, headers=None
        )
