...
class MCPResourceDefinition(BaseModel):
    ...
    supports_streaming: bool = Field(default=False, description="Supports streaming updates")
    supports_polling: bool = Field(default=False, description="Supports polling updates")


class BaseMCPResource(ABC):
    """Abstract base class for MCP resources with streaming/polling support."""
    
    def __init__(self, name: str, description: str, mime_type: str = "application/json"):
        self.name = name
    ...
    def subscribe(self, callback: Callable[[ResourceUpdate], None]) -> None:
        ...
        logger.info("Subscriber added", resource=self.name, total_subscribers=len(self._subscribers))
    
    def unsubscribe(self, callback: Callable[[ResourceUpdate], None]) -> None:
        """Unsubscribe from resource updates."""
        self._subscribers.discard(callback)
        logger.info("Subscriber removed", resource=self.name, total_subscribers=len(self._subscribers))
    ...
    async def start_streaming(self, **kwargs) -> None:
        ...
        if not self.supports_streaming():
            raise NotImplementedError(f"Resource {self.name} does not support streaming")
...
class MCPResourceRegistry:
    ...
    async def start_all_streaming(self, **kwargs) -> None:
        ...
        logger.info("Starting streaming for all resources", count=len(streaming_resources))
        ...
        if failures:
            logger.warning("Some streaming resources failed to start", failures=len(failures))
    ...
    async def stop_all_streaming(self) -> None:
        ...
        logger.info("Stopping streaming for all resources", count=len(streaming_resources))
    ...
    async def start_all_polling(self, interval_seconds: float = 30.0, **kwargs) -> None:
        ...
        logger.info("Starting polling for all resources", count=len(polling_resources), interval=interval_seconds)
        
        tasks = []
        for resource in polling_resources:
            task = asyncio.create_task(resource.start_polling(interval_seconds, **kwargs))
            tasks.append(task)
        ...
        failures = [r for r in results if isinstance(r, Exception)]
        if failures:
            logger.warning("Some polling resources failed to start", failures=len(failures))
